...
```bash
Untappd-Alerts$ sam build --use-container
```
...
```bash
Untappd-Alerts$ sam logs -n StockCheckerFunction --stack-name "untappd-alerts" --tail
```
...
```bash
Untappd-Alerts$ pip install -r tests/requirements.txt --user
# unit test
Untappd-Alerts$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
Untappd-Alerts$ AWS_SAM_STACK_NAME="untappd-alerts" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "untappd-alerts"
```
